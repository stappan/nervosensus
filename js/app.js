                    ...
                    ${(() => {
                        ...
                        if (rels.equivalences.length > 0) {
                            html += '<div class="asserted-equiv-section"><div class="asserted-label">Consistent with:</div><div class="related-cells">' +
                                rels.equivalences.map(r => `<a href="#cell/${r.idx}" class="related-cell-btn equiv-btn" onclick="event.stopPropagation();">${r.direction === 'to' ? '→' : '←'} ${r.label}</a>`).join('') + '</div></div>';
                        }
                        if (rels.subtypeOf.length > 0) {
                            html += '<div class="asserted-subtype-section"><div class="asserted-label">Proposed subtype of:</div><div class="related-cells">' +
                                rels.subtypeOf.map(r => `<a href="#cell/${r.idx}" class="related-cell-btn subtype-btn" onclick="event.stopPropagation();">↑ ${r.label}</a>`).join('') + '</div></div>';
                        }
                        if (rels.hasSubtypes.length > 0) {
                            html += '<div class="asserted-subtype-section"><div class="asserted-label">Has proposed subtypes:</div><div class="related-cells">' +
                                rels.hasSubtypes.map(r => `<a href="#cell/${r.idx}" class="related-cell-btn subtype-btn" onclick="event.stopPropagation();">↓ ${r.label}</a>`).join('') + '</div></div>';
                        }
                        return html + '</div>';
                    })()}
                    <a href="#cell/${idx}" style="display:inline-block;margin-top:1rem;padding:0.5rem 1rem;background:#667eea;color:white;border:none;border-radius:6px;cursor:pointer;font-weight:600;text-decoration:none;" onclick="event.stopPropagation();">View Full Details</a>
                </div>
            </div>
        </div>`;
    }).join('');
...
function buildCellDetailHTML(idx, useLinks) {
    ...
    if(ct.relatedCells&&ct.relatedCells.length>0){
        const relButtons = ct.relatedCells.map(rc => {
            if (useLinks) {
                const ri = CELL_TYPES.findIndex(x => x.preferredLabel === rc.label);
                return ri !== -1 ? `<a href="#cell/${ri}" class="related-cell-btn" onclick="event.stopPropagation();">${rc.label}</a>` : `<span class="related-cell-btn">${rc.label}</span>`;
            }
            return `<button class="related-cell-btn" onclick="showModalByName('${rc.label.replace(/'/g,"\\'")}')">${rc.label}</button>`;
        }).join('');
    ...
    if(hasAnyRelationship){
        ...
        if(relationships.equivalences.length > 0) {
            const equivLinks = relationships.equivalences.map(r => {
                ...
                if (useLinks) return `<a href="#cell/${r.idx}" class="related-cell-btn equiv-btn" title="${title}">${arrow} ${r.label}</a>`;
        ...
        if(relationships.subtypeOf.length > 0) {
            const subtypeLinks = relationships.subtypeOf.map(r => {
                if (useLinks) return `<a href="#cell/${r.idx}" class="related-cell-btn subtype-btn">↑ ${r.label}</a>`;
...
function buildRelatedSourceCellsHTML(idx) {
    ...
    return `<div class="detail-section"><h3>📋 Other cells from ${ct.sourceNomenclatureLabel} (${sourceCells.length})</h3><div class="related-source-cells">${sourceCells.map(item => {
        ...
        return `<a href="#cell/${item.idx}" class="related-source-cell-link"><div class="related-source-cell-name">${c.preferredLabel}</div><div class="related-source-cell-meta"><span class="related-source-cell-species">${c.species}</span>${axonBadge}</div></a>`;
    }).join('')}</div></div>`;
}
...
function showModal(idx) {
    ...
    if (detailLink) {
        detailLink.innerHTML = `<a href="#cell/${idx}" class="modal-detail-link">Open full detail</a><button class="modal-newtab-btn" onclick="window.open(location.pathname+location.search+'#cell/${idx}','_blank')" title="Open in new tab">↗</button>`;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle initial hash (e.g. direct link to #cell/5)
    if (location.hash.match(/^#cell\/\d+$/)) {
        handleHashChange();
    }
});
